...
date: "2026-01-01"
output-dir: ../../static/slides/ch4-quarto
...
4. [Assumptions for unbiased estimates](#assumptions-for-unbiased-estimates)
...
**Is this relationship meaningful, or just confounded?**
...
**The lesson:** A model can fit the data well (high R²) but still yield biased estimates. This is why we need [assumptions]{.alert}.
...
1. [**Zero conditional mean**]{style="color: #19375F;"}: $E[u_i | X_i] = 0$
...
3. [**Finite fourth moments**]{style="color: #19375F;"}: Large outliers are unlikely (finite kurtosis)
...
E[u_i | X_i] = 0
$$
The expected value of the error term is zero, [after conditioning on $X$]{.alert}.
:::
...
- If, for example, $X = education$, then we could (in principle) figure out its distribution in the population of adults

. . .

- Suppose (for simplicity), that $u$ is ability (or age or gender or martial status, etc.). $u$ will also have a distribution in the population 
...
- And this is the [zero conditional mean assumption]{.alert}


::: {.callout-warning}
## This is the KEY assumption
If $E[u|X] \neq 0$, then $\hat{\beta}_1$ is [biased]{.alert} — it systematically estimates the wrong thing. 

This is called [**omitted variable bias**]{style="color: #19375F;"}.
:::

## Visualizing Zero Conditional Mean {.smaller}

![](./ch4_figures/ch4_zero_conditional_mean.png){width=55% height=auto fig-align="center"}
...
This shows that the [population regression function]{.alert} is a linear function of X! 
...
![](./ch4_figures/ch4_ovb.png){width=75% height=auto fig-align="center"}
...
- By the CLT, $\hat{\beta}_1$ is approximately normal for large $n$
     ...
     - Usually, we are quite happy with $n > 100$
...
=
\frac{1}{n}
\frac{\operatorname{Var}\!\left[(X_i - \mu_X) u_i\right]}
...
1. We minimize the sum of squared residuations to estimate $\beta_1$ and $\beta_0$
2. Slopes have precise interpretations: $\Delta Y$ per unit $\Delta X$, holding $u$ fixed
3. $R^2$ and SER measure model fit, but don't relate to causality
4. Zero conditional mean is THE critical assumption for unbiased estimation
