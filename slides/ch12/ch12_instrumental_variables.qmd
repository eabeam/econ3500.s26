...
## Doing 2SLS Manually {.smaller}

To understand the two stages, we can do them by hand:

![](ch12_figures/ch12_stata_1.png){fig-align="center"}

## Manual 2SLS: A Warning {.smaller}
...
## Building Up: State FE Only {.smaller}

**Step 1**: `xtreg packpc avgprs incomepop, vce(cluster state) fe`

![](ch12_figures/ch12_stata_4.png){fig-align="center"}

Price coefficient: -0.355. Still suffers from simultaneous causality bias.

## Building Up: State + Year FE {.smaller}

**Step 2**: `xtreg packpc avgprs incomepop i.year, vce(cluster state) fe`

![](ch12_figures/ch12_stata_5.png){fig-align="center"}

Price coefficient: -0.416. Still biased --- price responds to state and time demand shocks.

## Adding the Instrument: First Stage {.smaller}

**Step 3**: `xtivreg packpc (avgprs = taxs) incomepop y1995, vce(cluster state) fe first`

![](ch12_figures/ch12_stata_6.png){fig-align="center"}

$F(3, 47) = 3001.56$ --- **very strong** first stage.

## Adding the Instrument: Second Stage {.smaller}

![](ch12_figures/ch12_stata_7.png){fig-align="center"}

Price coefficient: -0.409 (p < 0.001). Income: -1.68 (p = 0.048).

## Using Logarithms for Elasticities {.smaller}

$$\log(sales_{it}) = \alpha_i + \lambda_t + \beta_1 \log(price_{it}) + \beta_2 \log(income_{it}) + \delta V_i + \omega_{it}$$

Using logs lets us interpret $\beta_1$ as an **elasticity**. Instrument: log(sales tax).

![](ch12_figures/ch12_stata_8.png){fig-align="center"}
...
2. **Exogenous?** Debatable --- is quarter of birth correlated with innate ability or other wage determinants? (Bound, Jaeger, and Baker, 1993)
...
![](ch12_figures/ch12_stata_13.png){width=95% fig-align="center"}
...
## Bound, Jaeger, and Baker (1993): Two Critiques {.smaller}
