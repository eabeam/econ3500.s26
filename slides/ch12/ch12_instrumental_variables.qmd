...
::: {.callout-important}
## Endogeneity
All three problems result in $E(u_i | X_i) \neq 0$ --- a violation of the zero conditional mean assumption. When this happens, OLS is [biased and inconsistent]{.alert}.
:::
...
- IV can eliminate bias when $E(u_i | X_i) \neq 0$ using an [instrumental variable]{.kw} $Z$

## Motivating Example: Wages and Schooling {.smaller}
...
::: {.callout-tip}
## Key Insight
If you can hold demand fixed and only observe supply shifts, the equilibrium points trace out the **demand curve**. This is the intuition behind IV.
:::
:::
:::
...
## Example Instrument: Distance to College {.smaller}
...
- Distance from high school to nearest college:
  - [Powerful]{.kw}: Positively correlated with schooling attainment
  - [Exogenous]{.kw}: Uncorrelated with worker ability ($V$)
  - [Excluded]{.kw}: Growing up near a college doesn't directly *cause* higher wages
...
- **Sales tax** in state $i$?
  ...
  - **Exogenous**: No obvious reason why sales tax should be correlated with omitted demand factors
  - **Excluded**: No obvious reason why sales tax would directly affect demand, other than through price
...
A 1-unit increase in price is associated with a decrease of 0.42 packs per capita.
...
::: {.callout-important}
## Rule of Thumb: F > 10
The first-stage F-statistic should be **greater than 10**. If so, the instruments are [powerful]{.kw} (relevant).
:::
...
## Building Up: No Instruments {.smaller}
...
- Logarithms correct for inflation --- a dollar is worth less over time
...
## Example 1: Effect of Studying on Grades {.smaller}
...
**Results**: OLS coefficient on study = 0.038; IV coefficient = **0.36** (much larger). Why? Selection bias --- students who study more are not innately better students.

## Example 2: Returns to Schooling {.smaller}
...
1. **Powerful?** Yes --- quarter of birth is correlated with schooling attainment through compulsory schooling laws
2. **Exogenous?** Debatable --- is quarter of birth correlated with ability? (Known as the "weak instruments" concern with this study)
3. **Excludable?** Is there a direct effect of birth quarter on wages? Possibly through age effects
:::
...
- Report first-stage F-stat (must be > 10)
...
4. **First-stage F > 10** is the key diagnostic for instrument strength
