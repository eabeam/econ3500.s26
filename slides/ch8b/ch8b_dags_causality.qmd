...
When we say "control for $Z$" in a regression, we are [comparing units that have the same value of $Z$]{.kw}.

. . .

::: {.callout-note}
## Intuition
Controlling for $Z$ removes the variation in $X$ and $Y$ that is driven by $Z$. What's left is the variation in $X$ and $Y$ that comes from *other* sources.
:::

. . .

**In DAG terms:** Controlling for a variable [blocks all paths]{.kw} that pass through it.

- If $Z$ is on a **backdoor path**: controlling closes it → [removes bias]{.kw}
- If $Z$ is a **collider**: controlling opens it → [creates bias]{.alert}
- If $Z$ is a **mediator**: controlling blocks part of the causal effect → [removes what you want]{.alert}
...
## Wine and Health: Your Turn

Now it's your turn to apply DAG thinking to this question.

**Research question:** Does wine consumption improve heart health?

Use the worksheet handout to:
