...
**Takeaway:** You [cannot]{.alert} interpret $\beta_1$ or $\beta_2$ in isolation. Always compute the [marginal effect at a specific value of $X$]{.kw}. You should also know how to find the [turning point]{.alert}: $X^* = -\beta_1 / 2\beta_2$.
...
## DiD: Calculating Group Means {.smaller}
...
|:---|:---|:---|
| [Control]{.kw} ($\text{Treat}=0$) | $\beta_0$ | $\beta_0 + \beta_2$ |
| [Treatment]{.kw} ($\text{Treat}=1$) | $\beta_0 + \beta_1$ | $\beta_0 + \beta_1 + \beta_2 + \beta_3$ |
