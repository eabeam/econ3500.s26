...
## Causal Diagrams (DAGs): Review

A [directed acyclic graph (DAG)]{.kw} maps out the causal relationships between variables.

- **Directed**: Arrows point from cause → effect
- **Acyclic**: No loops — causality flows forward
- **Graph**: Nodes (variables) and edges (arrows)

. . .

::: {.callout-important}
## Why DAGs Matter for This Exam
DAGs give a [visual framework]{.alert} for diagnosing the five threats — especially OVB, simultaneous causality, and measurement error.
:::

## Three Types of Paths {.smaller}
...
::: {.column width="40%"}
![](../ch8b/ch8b_figures/ch8b_unobserved_confounder.png){width=100% fig-align="center"}

::: {.smaller}
[Gray node]{style="color: #708090; font-weight: bold;"} = unobserved
:::
:::

::: {.column width="55%"}
Every case of [omitted variable bias]{.kw} is an open backdoor path:

- Education ← Ability → Earnings
- Ability is unobserved → can't control for it
- OLS picks up causal effect **+** confounding bias
...
Controlling for a variable [blocks all paths]{.kw} through it. This can be good or bad:
...
::: {.column width="35%"}
![](../ch8b/ch8b_figures/ch8b_knowledge_check_1.png){width=100% fig-align="center"}
:::
...
1. List all paths from Insurance to Health
...
- Insurance → Health (causal — leave open)
- Insurance ← Income → Health (backdoor — [control for Income]{.alert})
...
::: {.column width="35%"}
![](../ch8b/ch8b_figures/ch8b_collider.png){width=100% fig-align="center"}
:::
...
Should you study this relationship [among hired workers only]{.alert}?
:::
:::
...
**No!** Conditioning on Job Hiring (the collider) [opens]{.alert} the path between Talent and Connections.

Among the hired, low Talent becomes correlated with high Connections — a spurious association.

This is [collider bias]{.kw} (a.k.a. selection bias).

## DAGs and Our Econometric Toolkit {.smaller}

DAGs help us see **why** each method works:

| Problem (in DAG terms) | Method | How it helps |
|:---|:---|:---|
| Open backdoor via [observed]{.kw} confounder | Add control variable | Closes the backdoor path |
| Open backdoor via [time-invariant unobserved]{.kw} confounder | Fixed effects (Ch 10) | Absorbs all $\alpha_i$ variables |
| Open backdoor via [any unobserved]{.kw} confounder | IV / 2SLS (Ch 12) | Isolates exogenous variation in $X$ |
...
With panel data, observations within the same entity are likely correlated over time. Use [clustered standard errors]{.alert} (cluster at the entity level) — otherwise SEs are too small and you over-reject.
:::
...
## The Wald Estimator (Binary Instrument) {.smaller}
...
|:---|:---|:---|
| [Reduced form]{.kw} | $Y$ on $Z$ | Total effect of $Z$ on $Y$ (includes the path through $X$) |
| [First stage]{.kw} | $X$ on $Z$ | Effect of $Z$ on $X$ |
| [IV estimate]{.kw} | Ratio | Causal effect of $X$ on $Y$, scaled by how much $Z$ moves $X$ |
