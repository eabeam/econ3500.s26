...
**In sum:**
- **Before sampling:** Our procedure has 95% coverage probability
- **After sampling:** The interval either contains the parameter or it doesn’t
...
![](./ch7_figures/ch7_stata_regression_base.png){width=35% fig-align="center"}

- Do French speakers have the same GPA as English speakers? → [Type 1]{.kw}: test $\beta_{french} = 0$]

. . .

- Do "Other" speakers have the same GPA as English speakers? → [Type 1]{.kw}: test $\beta_{other} = 0$
...
![](./ch7_figures/ch7_stata_unrestricted.png){width=55% fig-align="center"}
...
![](./ch7_figures/ch7_stata_restricted.png){width=55% fig-align="center"}
...
$$y_i = \beta_0 + u_i$$
(Just the sample mean — no predictors at all)
...
![](./ch7_figures/ch7_stata_overall_ftest.png){width=55% fig-align="center"}
...
## Aside: Homoskedasticity, F-Tests, and BLUE

We've seen homoskedasticity come up twice now:

1. **Ch5:** Needed for Gauss-Markov (OLS is BLUE)
2. **Ch7 (today):** The hand-computed $F$-statistic formula assumes it
...
| Context | Homoskedasticity needed? |
|---|---|
| OLS point estimates ($\hat{\beta}$) | No — unbiased either way |
| OLS is BLUE (most efficient) | **Yes** — the "B" in BLUE |
| Homoskedastic-only SEs / F-stats | **Yes** — wrong if violated |
| Robust SEs / robust F-stats | No — valid either way |
:::
...
## GiveDirectly Context: Why Cash Transfers? {.smaller}
...
- **Outcome:** Monthly household consumption (PPP-adjusted, KES)
- **Controls:** Female household head, log household size, age of head (in decades)

**Interaction Terms Used**

- `treatment × eligible` captures direct effect
- `treatment × ineligible` captures spillover effect
- Can test equality of these effects (Type 2 test)
