...
$$
\begin{aligned}
Y_i &= (\beta_0 + \color{#008080}{\beta_2}\delta_0) + (\beta_1 + \color{#008080}{\beta_2}\delta_1) X_{1i} + (\color{#008080}{\beta_2}v_i + u_i)
\end{aligned}
$$
...
$$
\begin{aligned}
wage &= \beta_0 + \beta_1 \cdot educ + \color{#008080}{\beta_2}(\color{#008080}{\delta_0 + \delta_1 \cdot educ + v}) + u \\
     &= (\beta_0 + \color{#008080}{\beta_2\delta_0}) + (\beta_1 + \color{#008080}{\beta_2\delta_1}) \cdot educ + (\color{#008080}{\beta_2v} + u)
\end{aligned}
$$
...
- The return to education $\beta_1$ will be [overestimated]{.alert} because $\color{#008080}{\beta_2\delta_1 > 0}$
...
- Bias = $\beta_2 \times \delta_1 > 0$ $\Rightarrow$ [Positive bias]{.alert} {.fragment}
...
In Chapter 5, we learned that OLS is [BLUE]{style="color: #19375F;"} (Best Linear Unbiased Estimator) under the Gauss-Markov conditions.

The same result extends to multiple regression — we just need [one more assumption]{.alert}:
...
1. **Zero conditional mean:** $E[u_i | X_{1i}, \ldots, X_{ki}] = 0$
2. **i.i.d. sampling**
3. **Large outliers rare** (finite fourth moments)
4. **No perfect multicollinearity**
5. **Homoskedasticity:** $\text{Var}(u_i | X_{1i}, \ldots, X_{ki}) = \sigma^2_u$
:::

. . .

- Assumptions 1–4 → OLS is [unbiased]{.kw}
- Add assumption 5 → OLS is [efficient]{.kw} (smallest variance among linear unbiased estimators)
