...
Unlike in an RCT (where controls are purely about precision), in DiD the parallel trends assumption may only hold *after conditioning* on observables. Omitting those variables is [OVB]{.alert}.

**Example:** Controlling for house characteristics (square footage, lot size) when estimating the incinerator effect — if newer homes were built farther from the site during this period, controls aren't just precision, they're identification.
:::
...
![Groups were converging before treatment. DiD attributes the continued convergence to the treatment — overstating the true effect.](../ch10b/ch10b_figures/ch10b_parallel_violated.png){width=65% fig-align="center"}
...
4. **No obvious differential shocks**: No major PA-specific event in 1992 that would have changed fast-food employment independent of the wage increase
:::
