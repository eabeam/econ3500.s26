...
Autocorrelation does not violate Assumption 2, which only requires independence across entities, not within.
:::
...
**Block-diagonal error structure** (2 entities × 3 periods):
...
- **Two-way clustering:** Some settings cluster on *both* entities and time simultaneously (`vce(twoway cluster entity time)` in Stata). Use when you suspect both within-entity serial correlation *and* cross-sectional dependence within a period.
- **Minimum clusters:** Clustered SEs perform poorly with [fewer than ~30–50 clusters]{.alert}. With fewer, consider wild bootstrap (`boottest` in Stata).

Clustered SEs do not change the point estimates — only the standard errors. They are more conservative (usually larger) than unclustered SEs when within-entity errors are positively correlated.
:::

# Staggered Adoption and Recent Developments

## Motivating Example: Castle Doctrine Laws {.smaller}
...
- Treatment: castle doctrine law in effect
...
When treatment effects are [homogeneous]{.kw} across states and stable over time, TWFE gives the right answer. But what if the effect in Florida looks different from the effect in Texas — or if the effect grows over time after adoption?

## The Treatment Timing Is Staggered {.smaller}

:::: {.columns}
::: {.column width="42%"}
![](assets/castle/castle_table1.png){width=100%}
:::

::: {.column width="58%"}
![](assets/castle/castle_data_desc.png){width=100%}
:::
::::
...
![](assets/castle/castle_bacon.png){width=78%}
...
- So this is a useful cautionary example: the estimator is still a weighted average of many 2x2 DiDs, even though the clean treated-vs-never-treated comparisons dominate here
