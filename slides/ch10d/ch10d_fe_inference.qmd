...
Castle Doctrine is the kind of application where this issue arises: some states adopt early, others later, and TWFE starts reusing already-treated states as comparisons.
...
- These are not weights the researcher chooses by hand.
...
![](assets/castle/castle_bacon_generated.png){width=88%}
...
In Stevenson and Wolfers on unilateral divorce, the Bacon decomposition is much messier. The problematic [timing-group comparisons]{.alert} get [more than one-third of the total weight]{.alert}, so the overall TWFE/DD estimate is much more exposed to contaminated treated-vs-treated comparisons. That is the kind of setting where staggered-adoption TWFE can become seriously misleading.
:::
