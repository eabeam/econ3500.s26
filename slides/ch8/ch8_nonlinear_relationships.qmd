...
::: {.callout-note}
## Overfitting
A model [overfits]{.alert} when it fits the sample data too closely — capturing noise rather than the true underlying relationship — and therefore predicts poorly on new data. Higher-order polynomials with too many terms are a classic example. *(S&W Ch. 8)*
:::
...
- Men in private sector: \$25 (baseline)
- Men in public sector: $25 + 3 = \$28$
- Women in private sector: $25 - 4 = \$21$ (gap = \$4)
- Women in public sector: $25 - 4 + 3 + 2 = \$26$ (gap = \$2)

. . .

$\hat{\beta}_3 = 2$: the gender wage gap is \$2 [smaller]{.alert} in the public sector. Test $H_0: \beta_3 = 0$ to check if the gap differs significantly.
...
[**Mistake 3: Forgetting to center variables**]{.alert}

If $X_2 = 0$ is not meaningful (e.g., zero years of experience), center $X_2$ around its mean before interacting. Then $\beta_1$ gives the effect of $X_1$ at the average value of $X_2$, which is much more interpretable.

. . .

[**Mistake 4: Testing the wrong hypothesis**]{.alert}
...
::: {.callout-important}
## Rule
Never include an interaction term without also including both main effects ($X_1$ and $X_2$) — even if the main effects are not individually significant.
:::
...
![](./ch8_figures/ch8_interpretation_guide.png){width=90%}
