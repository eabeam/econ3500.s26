...
3. Use [`CRIME4.dta`](../materials/CRIME4.dta) for this exercise, and see example 13.9 in this poor-quality scanned [upload](../materials/example-13.9.pdf).
   ...
   c. Add the logs of each wage variable in the data set and estimate the model by fixed effects. How does including these variables affect the coefficient on the criminal justic variables in part (b)?
...
4. **SW-12.6** In an instrumental variable regression model with one reressor, $X_i$, and one instrument, $Z_i$, the regression of $X_i$ onto $Z_i$ has $R^2 = 0.05$ and $n = 100$. Is $Z_i$ a strong instrument?[^4] Would your answer change if $R^2 = 0.05$ and $n = 500$?
